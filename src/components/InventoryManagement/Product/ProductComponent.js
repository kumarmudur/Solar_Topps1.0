...
import { productValidation, assemblyValidation } from '../../../util/validations';
import { compareObjects, checkNumeric, checkAlphanumeric, setDateFormat } from '../../../util/utility';
import { styles } from '../../../util/styles';
import { PRODUCT_SPECIFICATION, USERS } from '../../../constants/constants';
...
import { productGetInitialState, productSetInitialState, productAssemblyQuantityFieldChange, clearProductAssembly } from '../../../util/product';
...
class ProductComponent extends Component {
   constructor(props) {
      ...
      this.state = {
        ...
        isShowWarranty: this._checkIsProductHasWarranty(props.productData ? props.productData : props),
        fields: productGetInitialState(props.productData ? props.productData : props),
        prevObj: productGetInitialState(props.productData ? props.productData : props),
        isAssembly: this._checkIsAssembly(props.productData ? props.productData : props),
        errors: {},
  ...
  componentWillReceiveProps(nextProps) {
   ...
   if(nextProps && nextProps.id && nextProps.productData && nextProps.page && this.contentEdit === true) {
     ...
     obj = {
       ...
       otherPrice: productData.otherPrice || '',
 ...
 _checkIsProductHasWarranty = (props) => {
  const { productHasWarranty } = props;
  return productHasWarranty === 'Yes' ? true : false;
}
 ...
 _callFieldsSetState = () => {
  ...
  _setSpecificationFileUpload = data => {
    ...
    _onChangeSelectCategory = e => {
      ...
      this._changeSelectValue(value, name);
      if(value === 'Assembly') {
        this.props.getModuleList();
    ...
    _onChangeSelectStockAvailability = e => {
      const { value, name } = e.currentTarget;
      this._changeSelectValue(value, name);
      this.setState({
    ...
    _onChangeSelectPriceType = e => {
      const { value, name } = e.currentTarget;
      this._changeSelectValue(value, name);
    };
  
    _onChangeSelectWarrantyType = e => {
      const { value, name } = e.currentTarget;
      this._changeSelectValue(value, name);
      this.setState({
        isShowWarranty: this.state.isShowWarranty
   ...
   _onChangeSelectWarrantyYear = e => {
    const { value, name } = e.currentTarget;
     ...
     if(manufactureDate) {
       ...
       let yyyy = currentDate.getFullYear() + parseInt(value);
       if(dd === 0) {
         mm = mm - 1;
    ...
    _onChangeSelectProductEntry = e => {
      const { value, name } = e.currentTarget;
      const  selectedValue = value;
      this._onFieldChangeProductEntry(selectedValue, name);
    }
    ...
    _addSpecification = e => {
      ...
      const { id } = e.currentTarget;
      if(id) {
        this._addProductSpecification(id, true);
      }
    }
    ...
    _addProductSpecification = (name, checked) => {
      ...
      if(checked) {
        if(name === PRODUCT_SPECIFICATION.CHECKDIMENSION) {
          const dimensionTypeList = dimensionType && dimensionType.fieldValues && dimensionType.fieldValues.map(type => {
              return {
                label: type,
                value: type
              }
          });
          listType = dimensionTypeList;
        } else if (name === PRODUCT_SPECIFICATION.CHECKWEIGHT) {
          const weightTypeList = weightType && weightType.fieldValues && weightType.fieldValues.map(type => {
              return {
                label: type,
                value: type
             }
          });
          listType = weightTypeList;
        } else if (name === PRODUCT_SPECIFICATION.CHECKCOLOR) {
          const colorTypeList = colorType && colorType.fieldValues && colorType.fieldValues.map(type => {
              return {
                label: type,
                value: type
            }
          });
          listType = colorTypeList;
        } else if (name === PRODUCT_SPECIFICATION.CHECKPOWER) {
          const powerTypeList = powerType && powerType.fieldValues && powerType.fieldValues.map(type => {
             return {
              label: type,
              value: type
            }
          });
          listType = powerTypeList;
        } else if (name === PRODUCT_SPECIFICATION.CHECKDOCUMENTUPLOAD) {
          const documentTypeList = documentType && documentType.fieldValues && documentType.fieldValues.map(type => {
              return {
                label: type,
                value: type
            }
          });
          listType = documentTypeList;
        }
    ...
    _onChangeSelectProductSpecification = e => {
      const { value, name } = e.currentTarget;
      this._onFieldChangeProductSpecification(value, name);
    }
    ...
    _onChangeSelectOtherPrice = e => {
     ...
     // let fieldName = indexValue && indexValue[1];
      ...
      otherPrices.map((price, index) => {
        ...
        otherPrice = parseFloat(otherPrice) + parseFloat(price.totalCost || 0);
      });
      fields.otherPrices = otherPrices;
      fields.otherPrice = parseFloat(otherPrice);
      this._callFieldsSetState();
      this._callCualateTotalAssemblyPrice();
    }
  
    _onChangeSelectSubCategory = e => {
      const { value, name } = e.currentTarget;
      const { configurationsList } = this.props;
      let { fields } = this.state;
      const { otherPrices } = fields;
    ...
    _onDeleteProductOtherPrice = e => {
      ...
      this.isAddFile = false;
      const { fields } = this.state;
      const { otherPrices, otherPrice } = this.state.fields;
      const { id } = e.currentTarget;
      let indexValue = id && id.split('-');

      let configId = indexValue && indexValue[0];
      let totalCost = indexValue && parseFloat(indexValue[1]);
      const filteredData = otherPrices.filter((p, index )=> parseInt(configId) !== index);
      fields.otherPrices = filteredData;
      let grandTotalValue = otherPrice - totalCost;
      fields.otherPrice = parseFloat(grandTotalValue);
      this._callFieldsSetState();
      this._callCualateTotalAssemblyPrice();
    }
    ...
    _callCualateTotalAssemblyPrice = () => {
      ...
      const { grandTotalAssemblyResidential, grandTotalAssemblyCommercial, grandTotalAssemblyNonProfit, grandTotalAssemblyIndustrial,
        otherPrice, grandTotalPartsResidential, grandTotalPartsCommercial, grandTotalPartsNonProfit,
      ...
      parseFloat(otherPrice ? otherPrice : 0) + parseFloat(grandTotalPartsResidential ? grandTotalPartsResidential : 0);
      fields['totalAssemblyCommercialPrice'] = parseFloat(grandTotalAssemblyCommercial ? grandTotalAssemblyCommercial : 0) + 
      parseFloat(otherPrice ? otherPrice : 0) + parseFloat(grandTotalPartsCommercial ? grandTotalPartsCommercial : 0);
      fields['totalAssemblyNonProfitPrice'] = parseFloat(grandTotalAssemblyNonProfit ? grandTotalAssemblyNonProfit : 0) + 
      parseFloat(otherPrice ? otherPrice : 0 ) + parseFloat(grandTotalPartsNonProfit ? grandTotalPartsNonProfit : 0);
      fields['totalAssemblyIndustrialPrice'] = parseFloat(grandTotalAssemblyIndustrial ? grandTotalAssemblyIndustrial : 0) + 
      parseFloat(otherPrice ? otherPrice : 0) + parseFloat(grandTotalPartsIndustrial ? grandTotalPartsIndustrial : 0);
      this._callFieldsSetState();
    }

      // Assenlby Business Logic
    _onChangeSelectAseemblyModule = e => {
      ...
      console.log('value....', value);
        ...
        moduleData.map(item => {
          productAssemblyModule.productName = value,
      ...
      _onChangeSelectAseemblyOptimizer = e => {
        const { value } = e.currentTarget;
        let optimizerData = {};
        ...
        optimizerData.map(item => {
          productAssemblyOptimizer.productName = value,
      ...
      _onChangeSelectAseemblyInverter = e => {
        const { value } = e.currentTarget;
        let inverterData = {};
        ...
        inverterData.map(item => {
          productAssemblyInverter.productName = value,
    ...
    _submitForm = () => {
        const { isAssembly, fields } = this.state;
        const { productData } = this.props;
        const productId = productData && productData.productId ? productData.productId : null;
        const status = productData && productData.status ? productData.status : null;
        const createdDateTime = productData && productData.createdDateTime ? productData.createdDateTime : null;
    ...
    // To do check for material ui datepicker
    // _onDateChange = date => {
    //     const { fields } = this.state;
    //     // const { warrantyYear } = fields;
    //     console.log('_onDateChange-- : ', date);

    //     if(date) {
    //        fields['manufactureDate'] = date;
    //        fields['warrantyExpiry'] = date;
    //        this.setState({
    //         fields
    //        });
    //     }
   ...
   render() {
      ...
      const { isShowDeclineModal, shouldRedirect } = this.state;

      const { isShowWarranty, fields, errors, isAssembly, subCategoryList, costList, inverters, optimizers, productPartsList } = this.state;
      const { productName, category, manufacturer, modelNumber, manufactureDate, promotion, productCost, residential, commercial, nonProfit, industrial, 
        ...
        productEntries, specifications, otherPrices, uploadImages, otherPrice, assemblyWatt, productAssemblyModule, productParts,
      ...
      errorNonProfit, errorIndustrial, errorQuantity, errorThreshold, errorPower, errorQunatityBox, errorThresholdMessage } = errors;
      const { classes, staticData, warehouseList, modules, configurationsList, productData } = this.props; 
      ...
      if(shouldRedirect) {
        return <Redirect to={ '/app/Inventory/Products/ManageProduct' } />;
       }
   return (
    <Fragment>
         <div className="page-heading">
            <h1 className="inline-block-element">{ productId ? 'Edit Product' : 'Add Product' }</h1>
         ...
         <div className="rct-block ">
            ...
            <div className="collapse show">
               <div className="rct-block-content product-main-wrapper product-date-picker">
               <form className={clsx(classes.container, 'product-general-wrapper')} noValidate autoComplete="off">
         ...
         { !isAssembly ? <div className="rct-block ">
                <div className="rct-block-title ">
                <h4>Product Specification</h4>
                </div>
                <div className="collapse show">
                <div className="rct-block-content product-main-wrapper">
                <form className={clsx(classes.container, 'product-specification-wrapper')} noValidate autoComplete="none">
        ...
        { !isAssembly ? <div className="rct-block ">
                ...
                <div className="collapse show">
                <div className="rct-block-content product-main-wrapper">
                <form className={clsx(classes.container, 'product-cost-wrapper')} noValidate autoComplete="none">
        ...
        { isAssembly ? <div className="rct-block ">
            ...
            <div className="collapse show">
               <div className="rct-block-content product-main-wrapper">
                    <form className={clsx(classes.container, 'product-assembly-wrapper')} noValidate autoComplete="none">
         ...
         { isAssembly ? <div className="rct-block">
            ...
            <div className="collapse show">
               <div className="rct-block-content product-main-wrapper">
                <form className={clsx(classes.container, 'product-parts-wrapper')} noValidate autoComplete="none">
        ...
        <div className="rct-block ">
            ...
            <div className="collapse show">
               <div className="rct-block-content product-main-wrapper">
               <form className={clsx(classes.container, 'product-other-price-wrapper')} noValidate autoComplete="none">
            ...
            <div className="collapse show">
               ...
               <form className={classes.container} noValidate autoComplete="none">
                    <ProductStockComponent 
                        ...
                        errorThresholdMessage={ errorThresholdMessage }
                    />
               </form>
               </div>
            </div>
        ...
        {/* Product Entry */}
        {
          !isAssembly ?  <div className="rct-block ">
            ...
            <div className="collapse show">
               <div className="rct-block-content product-main-wrapper">
               <form className={clsx(classes.container, 'product-entry-wrapper')} noValidate autoComplete="none">
...
export default compose(withStyles(styles, { name: "ProductComponent" }),)(ProductComponent);